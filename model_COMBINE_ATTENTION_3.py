...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(512 + 512 + 128, 512, 3, padding=1, bias=False),
            nn.BatchNorm2d(512),
    ...
    def forward(self, image, text1, text2, z = None):
        ...
        #####################################################
        ### concatenate the image and text1 for attention ###
        fusion = torch.cat((img_feat, text_feat1), dim=1)
        #####################################################

        #$$$$$$$$$$$$$$$$$$$ Attention $$$$$$$$$$$$$$$$$$
        attention_score = self.attention(fusion.permute(0, 2, 3, 1)) # N * 16 * 16 * 1
        # print attention_score.size()
        weight = attention_score.permute(0, 3, 1, 2)  #.view(-1, 1, 16, 16)  # N * 1 * 16 * 16
        # print weight.size()
        weight = weight.repeat(1, 512, 1, 1)    # N * 512 * 16 * 16

        #################################################
        # sigmod or softmax?
        # weight = F.softmax(attention_score.squeeze().view(-1, 16 * 16))
        # print weight.size()
        # weight = weight.view(-1, 16, 16)
        # print weight.size()
        ...
        img_feat_attention = img_feat * weight
        fusion = self.residual_blocks(img_feat_attention)
        ###########################################

        fusion = torch.cat(((1 - weight) * img_feat, fusion, text_feat2), dim=1)
        # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
