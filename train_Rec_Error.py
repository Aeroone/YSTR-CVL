...
if __name__ == '__main__':
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (img1, text1, len_text1, img2, text2, len_text2) in tqdm(enumerate(train_loader)):
            ...
            fake_loss = F.binary_cross_entropy_with_logits(fake_logit, ONES) + 1.0e-04 * reconstruction_loss

            avg_rec_loss += 1.0e-04 * reconstruction_loss.data[0]
        ...
        print('Epoch [%d/%d], D_match: %.4f, D_mismatch: %.4f, D_fake: %.4f, G_fake: %.4f, Reconstruction_loss: %.4f, KLD: %.4f'
                % (epoch + 1, config.num_epochs, avg_D_real_loss, avg_D_real_mismatch_loss, avg_D_fake_loss, avg_G_fake_loss, avg_rec_loss, avg_kld))
        ...
        f.write(str(avg_D_real_loss) + "," + str(avg_D_real_mismatch_loss) + "," + str(avg_G_fake_loss) + "," + str(avg_rec_loss) + "\n")
